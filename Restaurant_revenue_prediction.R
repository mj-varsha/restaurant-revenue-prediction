...
lin_yhat_int = predict(stepwise_interaction,newdata=test_data)
...
lin_int_imp = varImp(stepwise_interaction)
...
lin_interaction_data <- data.frame(
  Variable = c(
    "Number_of_Customers", 
    "Menu_Price", 
    "Marketing_Spend", 
...
# Plotting variable importance for stepwise regression
ggplot(lin_interaction_data, aes(x = reorder(Variable, Importance), y = Importance)) +
...
#RMSE plot
train_rmse_values <- data.frame(
  Model = c("Linear Regression", "Stepwise Regression", "Regression Tree", "Random Forest", "Gradient Boosting"),
  RMSE = c(57.5, 57.45, 51.41, 61.737, 59.0125),
  Dataset = "Train"
)
test_rmse_values <- data.frame(
  Model = c("Linear Regression", "Stepwise Regression", "Regression Tree", "Random Forest", "Gradient Boosting"),
  RMSE = c(lin_rmse, lin_int_rmse, rt_rmse, rf_rmse, gbm_rmse),
  Dataset = "Test"
)
# Combine the training and test RMSE values
combined_rmse_values <- rbind(train_rmse_values, test_rmse_values)
# Plot the comparison of RMSE values for both training and test datasets
ggplot(combined_rmse_values, aes(x = Model, y = RMSE, fill = Dataset)) +
  geom_bar(stat = "identity", position = position_dodge(), width = 0.7) +  # Use position_dodge for side-by-side bars
  geom_text(aes(label = round(RMSE, 2)), position = position_dodge(0.7), vjust = -0.3, size = 3) +  # Add values on top of the bars
  theme_minimal() +
  labs(title = "RMSE Comparison of Different Models (Train vs Test)",
       x = "Model",
       y = "RMSE") +
